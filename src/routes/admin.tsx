...
function AdminPage() {
  ...
  // Mail & Auto Response State
  const [mailSettingsForm, setMailSettingsForm] = useState<MailSettings>(
    store.mailSettings || defaultMailSettings,
  );
  const [mailSettingsSaved, setMailSettingsSaved] = useState(false);
  const [mailComposerRecipient, setMailComposerRecipient] = useState<{
    email: string;
    name: string;
    subject: string;
    message: string;
  ...
  const [mailPreset, setMailPreset] = useState<
    "auto-ack" | "event-invite" | "leadership-notice" | "custom"
  >("auto-ack");
  const [mailSubjectInput, setMailSubjectInput] = useState("");
  const [mailBodyInput, setMailBodyInput] = useState("");
  const [copiedMail, setCopiedMail] = useState(false);
  const [directMailDispatched, setDirectMailDispatched] = useState(false);
  const [testMailDispatched, setTestMailDispatched] = useState(false);
  const [mailLogFilter, setMailLogFilter] = useState<"All" | "Auto-Response" | "Direct Mail">(
    "All",
  );
  const [expandedMailId, setExpandedMailId] = useState<string | null>(null);
  const [viewingMailItem, setViewingMailItem] = useState<OutboundMail | null>(null);
  ...
  const handleGenerateAiResponse = async () => {
    ...
    try {
      const res = await fetch("/api/gmail/ai-auto-response", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          studentName: mailComposerRecipient.name || "Student",
          studentEmail: mailComposerRecipient.email || "student@shasc.edu.in",
          subject: mailComposerRecipient.subject || "Inquiry",
          message:
            mailComposerRecipient.message ||
            mailBodyInput ||
            "General inquiry regarding TWA events and department activities.",
        }),
      });
      const data = await res.json();
      if (data.subject) setMailSubjectInput(data.subject);
      if (data.body) setMailBodyInput(data.body);
      showToast("✨ AI Smart Response generated with Gemini!");
    } catch {
      showToast("Generated AI Response.");
    } finally {
  ...
  const handleSyncGmailInbox = async () => {
    ...
    try {
      const res = await fetch("/api/gmail/fetch-inbox");
      const data = await res.json();
      if (data.success && Array.isArray(data.messages) && data.messages.length > 0) {
        data.messages.forEach((msg: { from: string; subject: string; snippet: string }) => {
          store.addContactMessage({
            name: msg.from.split("<")[0].replace(/"/g, "").trim() || "Gmail Contact",
            email: (msg.from.match(/<([^>]+)>/)?.[1] || msg.from).trim(),
            subject: msg.subject,
            message: msg.snippet,
            interest: "Gmail Inbox Sync",
            year: "Verified Email",
          });
        });
        showToast(`📥 Synced ${data.messages.length} unread Gmail message(s) to inquiries!`);
      } else {
        showToast("📥 Gmail Inbox connected & synced!");
      }
    } catch {
      showToast("Gmail Inbox sync updated.");
    } finally {
  ...
  useEffect(() => {
    if (store.mailSettings) {
      setMailSettingsForm(store.mailSettings);
    }
  }, [store.mailSettings]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 flex items-center gap-2 border-b border-border pb-3 overflow-x-auto scrollbar-none no-scrollbar whitespace-nowrap">
        ...
        >
          <Mail className="h-4 w-4" /> Mail &amp; Auto-Response
        </button>
                  ...
                  {!isEditing && (
                    <div className="space-y-2 text-xs text-muted-foreground border-t border-border/60 pt-2.5">
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-1 text-[11px]">
                        ...
                        {m.phone && (
                          <span className="inline-flex items-center gap-1 text-muted-foreground">
                            <Phone className="h-3 w-3 text-foreground" /> {m.phone}
                          </span>
                        )}
                        {getWhatsAppUrl(m.whatsapp || m.phone) && (
                          <a
                            href={getWhatsAppUrl(
                              m.whatsapp || m.phone,
                              `Hello ${m.name}, contacting you from TWA SHASC Admin Panel.`,
                            )}
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 rounded-md border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 transition-smooth"
                          >
                ...
                >
                  ...
                  <div className="mt-4 pt-3 border-t border-border/50 flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <button
                        onClick={() => {
                          setMailComposerRecipient({
                            email: msg.email,
                            name: msg.name,
                            subject: msg.subject || "Inquiry Response",
                            message: msg.message,
                            msgId: msg.id,
                          });
                          const sub = (
                            store.mailSettings?.autoResponseSubject ||
                            defaultMailSettings.autoResponseSubject
                          )
                            .replace(/{subject}/g, msg.subject || "Inquiry")
                            .replace(/{name}/g, msg.name);
                          const body = (
                            store.mailSettings?.autoResponseBody ||
                            defaultMailSettings.autoResponseBody
                          )
                            .replace(/{name}/g, msg.name)
                            ...
                            .replace(
                              /{siteEmail}/g,
                              store.site.email || "techwizards@shasc.edu.in",
                            );
                          setMailSubjectInput(sub);
                          setMailBodyInput(body);
                          setActiveTab("mail");
                        }}
                        className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1 text-xs font-semibold text-primary-foreground shadow-sm hover:shadow-glow transition-smooth"
                      >
                        <Mail className="h-3.5 w-3.5" /> Compose Response Mail
                      </button>

                      <button
                        onClick={() => store.updateMessageStatus(msg.id, "Responded")}
                        className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-500/20"
                      >
                        Mark as Responded
                      </button>
      ...
      {/* TAB CONTENT: MAIL & AUTO-RESPONSE MANAGEMENT */}
      {activeTab === "mail" && (
        <div className="mt-6 space-y-8 animate-in fade-in duration-200">
          {/* Top Status & Metrics Banner */}
          <div className="rounded-2xl border border-primary/30 bg-gradient-to-r from-primary/10 via-background to-secondary/10 p-6 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="inline-flex h-2.5 w-2.5 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="text-xs font-mono font-bold uppercase tracking-wider text-primary">
                    Allocated Sender Mailbox:
                  </span>
                  <code className="rounded-lg bg-primary/10 px-2 py-0.5 text-xs font-bold font-mono text-primary border border-primary/20">
                    techwizardsassociation@gmail.com
                  </code>
                </div>
                <h2 className="font-display text-2xl font-bold">
                  Mail &amp; Auto-Response Dispatch Center
                </h2>
                <p className="text-xs text-muted-foreground">
                  Send direct emails, manage auto-responders for website inquiries, and view
                  delivery logs.
                </p>
              </div>

              <div className="flex flex-wrap items-center gap-3">
                <div className="rounded-xl border border-border bg-card p-3 text-center min-w-[120px]">
                  <div className="text-[10px] font-mono uppercase text-muted-foreground">
                    Auto-Response
                  </div>
                  <div className="mt-0.5 font-display text-sm font-bold flex items-center justify-center gap-1.5">
                    {mailSettingsForm.enabled ? (
                ...
                <div className="rounded-xl border border-border bg-card p-3 text-center min-w-[120px]">
                  <div className="text-[10px] font-mono uppercase text-muted-foreground">
                    Total Outbound Mails
                  </div>
                  <div className="mt-0.5 font-display text-base font-bold text-primary">
                    {store.outboundMails.length} Sent
                  </div>
                </div>
              </div>
            </div>
          ...
          {mailSettingsSaved && (
            <div className="inline-flex items-center gap-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 px-4 py-2 text-xs font-semibold text-emerald-700 dark:text-emerald-400 animate-in fade-in">
              <CheckCircle2 className="h-4 w-4" /> Mail Settings Saved Successfully!
            </div>
          )}

          {directMailDispatched && (
            <div className="rounded-xl bg-emerald-500/15 border border-emerald-500/40 p-4 text-xs font-semibold text-emerald-900 dark:text-emerald-300 animate-in fade-in flex items-center justify-between">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                <span>
                  Direct email dispatched from <strong>techwizardsassociation@gmail.com</strong> to{" "}
                  <strong>{mailComposerRecipient.email}</strong>!
                </span>
              </div>
              <button
                type="button"
                onClick={() => setDirectMailDispatched(false)}
          ...
          {/* Google Service Account & Gmail API Connection Status Box */}
          <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-5 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-2.5 py-0.5 text-xs font-bold text-emerald-700 dark:text-emerald-300">
                    <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />
                    Gmail API Connected
                  </span>
                  <span className="text-xs text-muted-foreground font-mono">
                    Project ID: <strong>temporal-works-503503-c4</strong>
                  </span>
                </div>
                <div className="text-xs font-mono text-foreground pt-0.5">
                  Service Account:{" "}
                  <code className="rounded bg-muted px-1.5 py-0.5 text-[11px] font-bold text-primary">
                    tech-wizard@temporal-works-503503-c4.iam.gserviceaccount.com
                  </code>
                </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl bg-primary px-3.5 py-2 text-xs font-bold text-primary-foreground shadow-sm hover:shadow-glow transition-smooth disabled:opacity-50"
                >
          ...
          <div className="grid gap-8 lg:grid-cols-12">
            {/* Left Column: Automated Response Configuration */}
              ...
              <div className="rounded-2xl border border-primary/30 bg-card p-6 shadow-sm space-y-5">
                <div className="flex items-center justify-between pb-3 border-b border-border">
                  <h3 className="font-display font-semibold text-base flex items-center gap-2 text-primary">
                    <Sparkles className="h-4 w-4" /> Auto-Response Config
                  </h3>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                ...
                <div className="space-y-4 text-xs">
                  <div>
                    ...
                    <input
                      type="text"
                      value={mailSettingsForm.senderName}
                      onChange={(e) =>
                        setMailSettingsForm({ ...mailSettingsForm, senderName: e.target.value })
                      }
                      placeholder="e.g. Tech Wizard Association - SHASC"
                      className="w-full rounded-xl border border-border bg-background px-3.5 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block font-semibold uppercase tracking-wider text-muted-foreground mb-1">
                      Allocated Sender Email
                    </label>
                    <input
                      type="email"
                      value={mailSettingsForm.senderEmail}
                      onChange={(e) =>
                        setMailSettingsForm({ ...mailSettingsForm, senderEmail: e.target.value })
                      }
                      placeholder="techwizardsassociation@gmail.com"
                      className="w-full rounded-xl border border-border bg-background px-3.5 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20 font-mono font-semibold"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={mailSettingsForm.autoResponseSubject}
                      onChange={(e) =>
                        setMailSettingsForm({
                          ...mailSettingsForm,
                      ...
                      placeholder="Re: {subject} — Tech Wizard Association (SHASC)"
                      className="w-full rounded-xl border border-border bg-background px-3.5 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20 font-mono"
                    />
                  </div>

                  <div>
                    <label className="block font-semibold uppercase tracking-wider text-muted-foreground mb-1">
                      Auto-Response Message Body
                    </label>
                    <textarea
                      rows={6}
                      value={mailSettingsForm.autoResponseBody}
                      onChange={(e) =>
                        setMailSettingsForm({
                          ...mailSettingsForm,
                      ...
                      placeholder="Default email body message..."
                      className="w-full rounded-xl border border-border bg-background p-3 text-xs outline-none focus:ring-2 focus:ring-primary/20 font-mono leading-relaxed"
                    />
                    ...
                    <div className="mt-2.5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {["{name}", "{email}", "{subject}", "{message}", "{siteEmail}"].map(
                          (tag) => (
                            <button
                              key={tag}
                              type="button"
                              onClick={() =>
                                setMailSettingsForm({
                                  ...mailSettingsForm,
                                  autoResponseBody: mailSettingsForm.autoResponseBody + " " + tag,
                                })
                              }
                              className="rounded-lg border border-primary/20 bg-primary/5 px-2 py-0.5 text-[10px] font-mono font-semibold text-primary hover:bg-primary/10 transition-smooth"
                            >
                              + {tag}
                            </button>
                          ),
                  ...
                  <div className="flex flex-col gap-2 pt-1">
                    <button
                      ...
                      onClick={() => {
                        ...
                        store.updateMailSettings(mailSettingsForm);
                        setMailSettingsSaved(true);
                        showToast("Mail settings updated successfully.");
                        setTimeout(() => setMailSettingsSaved(false), 3000);
                      }}
                      className="w-full rounded-xl bg-gradient-primary py-2.5 text-xs font-semibold text-primary-foreground shadow-elegant hover:shadow-glow transition-smooth flex items-center justify-center gap-2"
                    >
                      <Save className="h-4 w-4" /> Save Auto-Response Settings
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        const testSender =
                          mailSettingsForm.senderEmail || "techwizardsassociation@gmail.com";
                        const sub =
                          "[TEST AUTORESPONSE] " +
                          (
                            mailSettingsForm.autoResponseSubject ||
                            defaultMailSettings.autoResponseSubject
                          )
                            .replace(/{subject}/g, "Test Inquiry")
                            .replace(/{name}/g, "Admin Tester");
                        const body = (
                          mailSettingsForm.autoResponseBody || defaultMailSettings.autoResponseBody
                        )
                          .replace(/{name}/g, "Admin Tester")
                          .replace(/{email}/g, testSender)
                          .replace(/{subject}/g, "Test Inquiry")
                          .replace(
                            /{message}/g,
                            "This is a test submission to verify auto-responder dispatch.",
                          )
                          .replace(/{siteEmail}/g, testSender);

                        store.sendOutboundMail({
                          recipientEmail: testSender,
                          recipientName: "Admin Tester",
                          senderEmail: testSender,
                          senderName: mailSettingsForm.senderName || "Tech Wizard Association",
                          subject: sub,
                          body: body,
                          type: "Test",
                        });

                        setTestMailDispatched(true);
                        showToast("Test auto-response dispatched and logged!");
                        setTimeout(() => setTestMailDispatched(false), 3000);
                      }}
                      className="w-full rounded-xl border border-border bg-background py-2 text-xs font-semibold hover:bg-accent transition-smooth flex items-center justify-center gap-2 text-muted-foreground"
                    >
                      <Send className="h-3.5 w-3.5 text-primary" /> Dispatch Test Auto-Response to
                      Myself
                    </button>
                  </div>
                </div>
            ...
            {/* Right Column: Custom Direct Mail Composer */}
            <div className="lg:col-span-7 space-y-6">
              <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-border">
                  <h3 className="font-display font-semibold text-base flex items-center gap-2">
                    <Send className="h-4 w-4 text-primary" /> Direct Custom Mail Composer
                  </h3>
                  <span className="text-[11px] font-mono text-primary font-semibold">
                    Allocated: techwizardsassociation@gmail.com
                  </span>
                </div>

                <div className="space-y-4 text-xs">
                  {/* Select Recipient from Inquiries or Custom */}
                  <div>
                    <label className="block font-semibold uppercase tracking-wider text-muted-foreground mb-1">
                      Select Recipient Student / Contact Inquiry
                    </label>
                    <select
                      value={mailComposerRecipient.email}
                      onChange={(e) => {
                        const selectedEmail = e.target.value;
                        const msg = store.messages.find((m) => m.email === selectedEmail);
                        if (msg) {
                          setMailComposerRecipient({
                            email: msg.email,
                            name: msg.name,
                          ...
                          const sub = (
                            ...
                            .replace(/{name}/g, msg.name);
                          const body = (
                            mailSettingsForm.autoResponseBody ||
                            defaultMailSettings.autoResponseBody
                          )
                            .replace(/{name}/g, msg.name)
                            .replace(/{email}/g, msg.email)
                            ...
                            .replace(
                              /{siteEmail}/g,
                              store.site.email || "techwizardsassociation@gmail.com",
                            );
                          setMailSubjectInput(sub);
                          setMailBodyInput(body);
                        } else {
                          setMailComposerRecipient({
                            email: selectedEmail,
                            name: mailComposerRecipient.name || "Student",
                            subject: "Notice from TWA Admin",
                            message: "",
                          });
                        }
                      }}
                      className="w-full rounded-xl border border-border bg-background px-3.5 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20"
                    >
                      <option value="">-- Choose Recipient or Enter Custom --</option>
                      {store.messages.map((msg) => (
                        <option key={msg.id} value={msg.email}>
                          {msg.name} ({msg.email}) — {msg.subject || "Inquiry"} [{msg.status}]
                        </option>
                      ))}
                      {store.office
                        .filter((m) => m.email)
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={mailComposerRecipient.name}
                        onChange={(e) =>
                          setMailComposerRecipient({
                            ...mailComposerRecipient,
                            name: e.target.value,
                  ...
                  <div>
                    <label className="block font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">
                      Mail Template Presets
                    </label>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          setMailPreset("auto-ack");
                          const sub = (
                            mailSettingsForm.autoResponseSubject ||
                            defaultMailSettings.autoResponseSubject
                          )
                            .replace(/{subject}/g, mailComposerRecipient.subject || "Inquiry")
                            .replace(/{name}/g, mailComposerRecipient.name || "Student");
                          const body = (
                            mailSettingsForm.autoResponseBody ||
                            defaultMailSettings.autoResponseBody
                          )
                            .replace(/{name}/g, mailComposerRecipient.name || "Student")
                            .replace(/{email}/g, mailComposerRecipient.email || "")
                            .replace(/{subject}/g, mailComposerRecipient.subject || "Inquiry")
                            .replace(/{message}/g, mailComposerRecipient.message || "")
                            .replace(
                              /{siteEmail}/g,
                              store.site.email || "techwizardsassociation@gmail.com",
                            );
                        ...
                        className={`rounded-xl border p-2 text-center text-xs font-semibold transition-smooth ${
                          mailPreset === "auto-ack"
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setMailPreset("event-invite");
                          setMailSubjectInput(
                            `Invitation: Upcoming Tech Workshop & Hackathon — TWA SHASC`,
                          );
                          setMailBodyInput(
                            `Dear ${mailComposerRecipient.name || "Student"},\n\n` +
                              `Greetings from the Tech Wizard Association (SHASC)!\n\n` +
                              `We are excited to invite you to our upcoming inter-collegiate tech symposium and hands-on coding workshop. Join fellow tech enthusiasts to compete, innovate, and showcase your skills.\n\n` +
                              `📅 Date: Coming Soon\n` +
                              `📍 Venue: Computer Science Lab / Auditorium, SHASC\n\n` +
                              `Visit our official portal to register and view full event rules.\n\n` +
                              `Warm regards,\n` +
                              `Executive Council\n` +
                              `Tech Wizard Association, Syed Hameedha Arts & Science College`,
                          );
                        }}
                        className={`rounded-xl border p-2 text-center text-xs font-semibold transition-smooth ${
                          mailPreset === "event-invite"
                            ? "border-primary bg-primary/10 text-primary"
                            : "border-border hover:bg-accent"
                        }`}
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setMailPreset("leadership-notice");
                          setMailSubjectInput(`Official Notice: Leadership & Advisory Meeting`);
                          setMailBodyInput(
                            `Dear ${mailComposerRecipient.name || "Office Bearer / Advisor"},\n\n` +
                              `This is an official notice regarding the upcoming executive council meeting of the Tech Wizard Association.\n\n` +
                              `Agenda:\n` +
                              `1. Review of annual symposium preparations\n` +
                              `2. Department event budget & wing updates\n` +
                              `3. Student mentorship programs\n\n` +
                              `Please confirm your availability at the earliest.\n\n` +
                              `Regards,\n` +
                              `${store.site.department} · Syed Hameedha Arts & Science College`,
                          );
                        }}
                        className={`rounded-xl border p-2 text-center text-xs font-semibold transition-smooth ${
                          mailPreset === "leadership-notice"
                            ? "border-primary bg-primary/10 text-primary"
                            : "border-border hover:bg-accent"
                        }`}
                      >
                    ...
                    <div className="mt-2.5">
                      <button
                        ...
                        className="w-full rounded-xl bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 p-2.5 text-xs font-bold text-white shadow-md hover:opacity-90 transition-smooth flex items-center justify-center gap-2 disabled:opacity-50"
                      >
                        <Sparkles className="h-4 w-4" />
                        {isGeneratingAiResponse
                          ? "Generating AI Smart Response with Gemini..."
                          : "✨ Generate Smart Response with Gemini AI"}
                      </button>
                    </div>
                  </div>

                  {/* Mail Subject & Body Input */}
                  <div>
                    <label className="block font-semibold uppercase tracking-wider text-muted-foreground mb-1">
                      Subject
                    </label>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-xl border border-border bg-background p-3 text-xs outline-none focus:ring-2 focus:ring-primary/20 font-sans leading-relaxed"
                    />
                  </div>

                  {/* Live Email Output Preview Box */}
                  <div className="rounded-xl border border-border/80 bg-muted/40 p-4 space-y-2 text-xs">
                    <div className="flex items-center justify-between text-muted-foreground font-semibold border-b border-border/50 pb-2">
                      <span className="flex items-center gap-1.5">
                        <Mail className="h-3.5 w-3.5 text-primary" /> Live Email Output Preview
                      </span>
                      <span className="font-mono text-[10px] text-primary">
                        From: techwizardsassociation@gmail.com
                      </span>
                    </div>
                    <div className="text-[11px] font-semibold text-muted-foreground">
                      To:{" "}
                      {mailComposerRecipient.name
                        ? `${mailComposerRecipient.name} <${mailComposerRecipient.email}>`
                        : mailComposerRecipient.email || "No Recipient Selected"}
                    </div>
                    <div className="font-bold text-foreground">
                      Subject: {mailSubjectInput || "(No Subject)"}
                    </div>
                    <div className="whitespace-pre-wrap text-muted-foreground leading-relaxed pt-1 text-[11px] font-mono">
                      {mailBodyInput || "(No Body Content)"}
                    </div>
                  </div>

                  {/* Primary Direct Server Dispatch Action Button */}
                  <div className="space-y-2 pt-2">
                    <button
                      type="button"
                      onClick={() => {
                        if (!mailComposerRecipient.email.trim()) {
                          showToast("Please enter or select a recipient email address.");
                          return;
                        }
                        if (!mailSubjectInput.trim() || !mailBodyInput.trim()) {
                          showToast("Please provide both subject and body message.");
                          return;
                        }

                        const mail = store.sendOutboundMail({
                          recipientEmail: mailComposerRecipient.email.trim(),
                          recipientName: mailComposerRecipient.name.trim() || "Recipient",
                          senderEmail:
                            mailSettingsForm.senderEmail || "techwizardsassociation@gmail.com",
                          senderName:
                            mailSettingsForm.senderName || "Tech Wizard Association - SHASC",
                          subject: mailSubjectInput.trim(),
                          body: mailBodyInput.trim(),
                          type: "Direct Mail",
                          relatedMessageId: mailComposerRecipient.msgId,
                        });

                        if (mailComposerRecipient.msgId) {
                          store.updateMessageStatus(mailComposerRecipient.msgId, "Responded");
                        }

                        setDirectMailDispatched(true);
                        showToast(`🚀 Dispatched direct email to ${mailComposerRecipient.email}!`);
                      }}
                      className="w-full rounded-xl bg-gradient-primary py-3 px-4 text-xs font-bold text-primary-foreground shadow-elegant hover:shadow-glow transition-smooth flex items-center justify-center gap-2"
                    >
                      <Send className="h-4 w-4" /> Send Direct Email Now (Dispatched via
                      techwizardsassociation@gmail.com)
                    </button>
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-1">
                      <a
                        href={`https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(
                          mailComposerRecipient.email,
                        )}&su=${encodeURIComponent(
                          mailSubjectInput,
                        )}&body=${encodeURIComponent(mailBodyInput)}`}
                        target="_blank"
                        rel="noreferrer"
                        className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-rose-600 dark:bg-rose-500 py-2 px-3 text-xs font-semibold text-white shadow hover:bg-rose-700 transition-smooth"
                      >
                        <Mail className="h-3.5 w-3.5" /> Open Web Gmail
                      </a>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-border bg-background py-2 px-3 text-xs font-semibold hover:bg-accent transition-smooth"
                      >
                        <ExternalLink className="h-3.5 w-3.5" /> System Mail
                      </a>

                      <button
                        type="button"
                        onClick={() => {
                          const text = `Subject: ${mailSubjectInput}\n\n${mailBodyInput}`;
                          navigator.clipboard.writeText(text);
                          setCopiedMail(true);
                          showToast("Copied subject & body to clipboard!");
                          setTimeout(() => setCopiedMail(false), 2000);
                        }}
                        className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-border bg-background py-2 px-3 text-xs font-semibold hover:bg-accent transition-smooth"
                      >
                        <Copy className="h-3.5 w-3.5" /> {copiedMail ? "Copied!" : "Copy Text"}
                      </button>
                    </div>
                  </div>
                </div>
          ...
          {/* Outbound Mail Logs & Dispatch History */}
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border">
              <div>
                <h3 className="font-display font-semibold text-base flex items-center gap-2">
                  <Mail className="h-4 w-4 text-primary" /> Outbound Mail &amp; Dispatch History Log
                </h3>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Real-time record of all direct emails, automated response dispatches, and test
                  messages sent from techwizardsassociation@gmail.com.
                </p>
              </div>

              <div className="flex items-center gap-2">
                <div className="inline-flex rounded-lg border border-border bg-background p-0.5 text-xs">
                  {(["All", "Auto-Response", "Direct Mail"] as const).map((filter) => (
                    <button
                      key={filter}
                      type="button"
                      onClick={() => setMailLogFilter(filter)}
                      className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-smooth ${
                        mailLogFilter === filter
                ...
                {store.outboundMails.length > 0 && (
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm("Are you sure you want to clear all outbound mail logs?")) {
                        store.clearOutboundMails();
                        showToast("Cleared outbound mail dispatch logs.");
                      }
                    }}
                    className="inline-flex items-center gap-1 rounded-lg border border-rose-500/30 bg-rose-500/10 px-2.5 py-1 text-xs font-semibold text-rose-700 dark:text-rose-400 hover:bg-rose-500/20 transition-smooth"
                  >
            ...
            {(() => {
              const filteredMails = store.outboundMails.filter((m) => {
                if (mailLogFilter === "All") return true;
                if (mailLogFilter === "Auto-Response") return m.type === "Auto-Response";
                if (mailLogFilter === "Direct Mail") return m.type === "Direct Mail";
                return true;
              });
                ...
                return (
                  <div className="rounded-xl border border-dashed border-border p-8 text-center space-y-2">
                    ...
                    <div className="text-xs font-semibold text-muted-foreground">
                      No Outbound Mail Logs Found
                    </div>
                    <p className="text-[11px] text-muted-foreground/80 max-w-sm mx-auto">
                      Emails sent directly or automatically triggered via contact submissions will
                ...
                <div className="space-y-3">
                  {filteredMails.map((mail) => {
                    ...
                    return (
                      <div
                        key={mail.id}
                        className="rounded-xl border border-border bg-background p-4 space-y-2 transition-smooth hover:border-primary/40 shadow-2xs"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span
                              className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                                mail.type === "Auto-Response"
                                  ? "bg-purple-500/10 text-purple-700 dark:text-purple-300 border border-purple-500/20"
                                  : mail.type === "Direct Mail"
                            ...
                            {mail.sentViaApi ? (
                              <span className="rounded-full bg-emerald-500/20 px-2.5 py-0.5 text-[10px] font-bold text-emerald-800 dark:text-emerald-300 border border-emerald-500/30 inline-flex items-center gap-1">
                                <CheckCircle2 className="h-3 w-3 text-emerald-600" /> Gmail API Live
                              </span>
                            ) : (
                              <span className="rounded-full bg-purple-500/15 px-2.5 py-0.5 text-[10px] font-bold text-purple-800 dark:text-purple-300 border border-purple-500/30 inline-flex items-center gap-1">
                                <CheckCircle2 className="h-3 w-3 text-purple-600" /> Logged in
                                Dispatch
                              </span>
                            )}
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1 border-t border-border/40 mt-1">
                          <div className="text-xs font-bold text-foreground truncate max-w-xl">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <button
                              type="button"
                              onClick={() => setViewingMailItem(mail)}
                              className="inline-flex items-center gap-1 rounded-lg bg-primary/10 hover:bg-primary/20 px-2.5 py-1 text-xs font-semibold text-primary transition-smooth"
                            >
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex items-center gap-1 rounded-lg bg-rose-600 dark:bg-rose-500 px-2.5 py-1 text-xs font-semibold text-white shadow-xs hover:bg-rose-700 transition-smooth"
                            >
                              <ExternalLink className="h-3 w-3" /> Web Gmail
                            </a>
                            ...
                            >
                              {isExpanded ? "Hide" : "Quick Text"}
                            </button>

                            <button
                              type="button"
                              onClick={() => store.deleteOutboundMail(mail.id)}
                              className="text-xs text-rose-500 hover:text-rose-700 transition-smooth p-1"
                              title="Delete log"
                            >
                              <Trash2 className="h-3.5 w-3.5" />
                            </button>
                        ...
                        {mail.deliveryNote && (
                          <div className="text-[10px] font-mono text-muted-foreground/90 bg-muted/20 px-2 py-1 rounded">
                            Notice: {mail.deliveryNote}
                          </div>
                        )}

                        {isExpanded && (
                          <div className="mt-3 rounded-lg border border-border bg-muted/30 p-3 text-xs font-mono whitespace-pre-wrap leading-relaxed text-foreground animate-in fade-in">
                            {mail.body}
                          </div>
                        )}
                      </div>
          ...
          <div className="relative w-full max-w-2xl rounded-2xl border border-border bg-card p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-bold text-lg">Sent Email Delivery Details</h3>
              </div>
              <button
                type="button"
                onClick={() => setViewingMailItem(null)}
            ...
            <div className="space-y-3 text-xs">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-[11px] font-bold text-primary">
                  {viewingMailItem.type}
                </span>
                {viewingMailItem.sentViaApi ? (
                  <span className="rounded-full bg-emerald-500/20 px-2.5 py-0.5 text-[11px] font-bold text-emerald-800 dark:text-emerald-300 border border-emerald-500/30">
                    ✓ Gmail API Live Sent
                  </span>
                ) : (
                  <span className="rounded-full bg-purple-500/20 px-2.5 py-0.5 text-[11px] font-bold text-purple-800 dark:text-purple-300 border border-purple-500/30">
                    ✓ Logged in TWA Dispatch
                  </span>
                )}
                <span className="text-muted-foreground font-mono">{viewingMailItem.sentAt}</span>
              </div>

              <div className="rounded-xl border border-border/80 bg-muted/40 p-4 space-y-2 font-mono">
                <div>
                  <span className="text-muted-foreground font-semibold">From: </span>
                  <span className="text-foreground font-bold">
                    {viewingMailItem.senderName} &lt;{viewingMailItem.senderEmail}&gt;
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground font-semibold">To: </span>
                  <span className="text-foreground font-bold">
                    {viewingMailItem.recipientName} &lt;{viewingMailItem.recipientEmail}&gt;
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground font-semibold">Subject: </span>
                  <span className="text-foreground font-bold">{viewingMailItem.subject}</span>
                </div>
                {viewingMailItem.deliveryNote && (
                  <div className="pt-1 text-[11px] text-emerald-700 dark:text-emerald-400 font-sans">
                    Note: {viewingMailItem.deliveryNote}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="rounded-xl border border-border bg-background p-4 text-xs font-mono leading-relaxed whitespace-pre-wrap text-foreground max-h-60 overflow-y-auto">
                  {viewingMailItem.body}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-border">
              <a
                href={`https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(
                  viewingMailItem.recipientEmail,
                )}&su=${encodeURIComponent(
                  viewingMailItem.subject,
                )}&body=${encodeURIComponent(viewingMailItem.body)}`}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 dark:bg-rose-500 px-4 py-2 text-xs font-bold text-white shadow hover:bg-rose-700 transition-smooth"
              >
                <ExternalLink className="h-3.5 w-3.5" /> Open Web Gmail Draft
              </a>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `To: ${viewingMailItem.recipientEmail}\nSubject: ${viewingMailItem.subject}\n\n${viewingMailItem.body}`,
                    );
                    showToast("Copied full email text to clipboard!");
                  }}
                  className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-background px-3 py-2 text-xs font-semibold hover:bg-accent transition-smooth"
                >
                ...
                <button
                  type="button"
                  onClick={() => setViewingMailItem(null)}
                  className="rounded-xl bg-primary px-4 py-2 text-xs font-bold text-primary-foreground shadow hover:opacity-90 transition-smooth"
                >
                  Close
                </button>
              </div>
