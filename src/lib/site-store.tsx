...
export function getWhatsAppUrl(phoneOrWa?: string, defaultMsg = "") {
  if (!phoneOrWa) return "";
  if (phoneOrWa.startsWith("http://") || phoneOrWa.startsWith("https://")) {
    return phoneOrWa;
  }
  const clean = phoneOrWa.replace(/[^0-9]/g, "");
  if (!clean) return "";
  const formattedNum = clean.length === 10 ? `91${clean}` : clean;
  const msgParam = defaultMsg ? `?text=${encodeURIComponent(defaultMsg)}` : "";
  return `https://wa.me/${formattedNum}${msgParam}`;
}
...
export interface OutboundMail {
  ...
  subject: string;
  body: string;
  type: "Auto-Response" | "Direct Mail" | "Custom Broadcast" | "Test";
  sentAt: string;
...
export const defaultMailSettings: MailSettings = {
  autoResponseSubject: "Re: {subject} — Tech Wizard Association (SHASC)",
  autoResponseBody:
    'Dear {name},\n\nThank you for reaching out to the Tech Wizard Association (Department of Computer Applications, Syed Hameedha Arts and Science College).\n\nWe have received your message regarding "{subject}". Our leadership team and advisors are reviewing your inquiry and will provide a detailed response shortly.\n\nSummary of your message:\n"{message}"\n\nWarm regards,\nTech Wizard Association (TWA)\nSyed Hameedha Arts & Science College, Kilakarai\nEmail: {siteEmail}',
  senderName: "Tech Wizard Association - SHASC",
  senderEmail: "techwizardsassociation@gmail.com",
  replyToEmail: "techwizardsassociation@gmail.com",
...
interface SiteStoreContextType {
  ...
  updateMailSettings: (info: Partial<MailSettings>) => void;
  sendOutboundMail: (mail: Omit<OutboundMail, "id" | "sentAt" | "status">) => OutboundMail;
  deleteOutboundMail: (id: string) => void;
  clearOutboundMails: () => void;
...
export function SiteStoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const unsubMail = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          const data = snapshot.data() as Partial<MailSettings>;
          setMailSettings((prev) => ({ ...prev, ...data }));
        } else {
          setDoc(doc(db, "settings", "mail"), defaultMailSettings).catch((err) =>
            handleFirestoreError(err, OperationType.WRITE, "settings/mail"),
          );
        }
      },
      (err) => console.warn("Firestore mail settings listener error", err),
    );
    ...
    // 7.5. Outbound Mails Collection
    const unsubOutbound = onSnapshot(
      collection(db, "outboundMails"),
      (snapshot) => {
        const items = snapshot.docs.map((d) => ({ id: d.id, ...d.data() }) as OutboundMail);
        items.sort((a, b) => new Date(b.sentAt).getTime() - new Date(a.sentAt).getTime());
        setOutboundMails(items);
      },
      (err) => console.warn("Firestore outboundMails listener error", err),
    );
  ...
  const applyParsedData = (parsed: Record<string, unknown> | null) => {
    ...
    if (Array.isArray(parsed.outboundMails)) {
      setOutboundMails(parsed.outboundMails as OutboundMail[]);
    }
  ...
  const saveAll = (overrides: Partial<typeof latestStateRef.current> = {}) => {
    try {
      ...
      const payload = {
        ...
        outboundMails: overrides.outboundMails ?? current.outboundMails,
  ...
  const updateMailSettings = (info: Partial<MailSettings>) => {
    setMailSettings((prev) => {
      const updated = { ...prev, ...info };
      saveAll({ mailSettings: updated });
      setDoc(doc(db, "settings", "mail"), updated, { merge: true }).catch((err) =>
        handleFirestoreError(err, OperationType.WRITE, "settings/mail"),
      );
      return updated;
    });
  ...
  const addContactMessage = (msg: Omit<ContactMessage, "id" | "date" | "status">) => {
    ...
    setMessages((prev) => {
      ...
      const cleanDoc = Object.fromEntries(
        Object.entries(newMsg).filter(([_, v]) => v !== undefined),
      );
      setDoc(doc(db, "messages", newMsg.id), cleanDoc).catch((err) =>
        handleFirestoreError(err, OperationType.WRITE, `messages/${newMsg.id}`),
      );
      return updated;
    });

    // Automatically trigger auto-response dispatch if enabled
    if (mailSettings.autoResponseEnabled || mailSettings.enabled) {
      const autoSub = (mailSettings.autoResponseSubject || defaultMailSettings.autoResponseSubject)
        .replace(/{subject}/g, msg.subject || "Inquiry")
        .replace(/{name}/g, msg.name);

      const autoBody = (mailSettings.autoResponseBody || defaultMailSettings.autoResponseBody)
        .replace(/{name}/g, msg.name)
        .replace(/{email}/g, msg.email)
        .replace(/{subject}/g, msg.subject || "Inquiry")
        .replace(/{message}/g, msg.message)
        .replace(/{siteEmail}/g, site.email || "techwizardsassociation@gmail.com");

      sendOutboundMail({
        recipientEmail: msg.email,
        recipientName: msg.name,
        senderEmail: mailSettings.senderEmail || "techwizardsassociation@gmail.com",
        senderName: mailSettings.senderName || "Tech Wizard Association - SHASC",
        subject: autoSub,
        body: autoBody,
        type: "Auto-Response",
        relatedMessageId: newMsg.id,
      });
    }
  };

  const sendOutboundMail = (
    mailData: Omit<OutboundMail, "id" | "sentAt" | "status">,
  ): OutboundMail => {
    const newMail: OutboundMail = {
      ...mailData,
    ...
    setOutboundMails((prev) => {
      const updated = [newMail, ...prev];
      saveAll({ outboundMails: updated });
      const cleanMailDoc = Object.fromEntries(
        Object.entries(newMail).filter(([_, v]) => v !== undefined),
      );
      setDoc(doc(db, "outboundMails", newMail.id), cleanMailDoc).catch((err) =>
        handleFirestoreError(err, OperationType.WRITE, `outboundMails/${newMail.id}`),
      );
      return updated;
    });

    // Fire API call to backend Gmail endpoint
    if (typeof window !== "undefined") {
      fetch("/api/gmail/send", {
        ...
        body: JSON.stringify({
          ...
          toName: mailData.recipientName,
          subject: mailData.subject,
          body: mailData.body,
          fromName: mailData.senderName,
          fromEmail: mailData.senderEmail,
        }),
      })
        .then((res) => res.json())
        .then((data) => {
          if (data && typeof data === "object") {
            const isLive = Boolean(data.sentViaApi);
            const noteText =
              data.note ||
              (isLive
                ? "Dispatched directly via Gmail API"
                : "Logged in TWA Outbound Dispatch System");
              ...
              const updated = prev.map((m) =>
                m.id === newMail.id
                  ? {
                      ...m,
                      sentViaApi: isLive,
                      deliveryNote: noteText,
                    }
                  : m,
              );
              saveAll({ outboundMails: updated });
              const target = updated.find((m) => m.id === newMail.id);
              if (target) {
                const cleanMailDoc = Object.fromEntries(
                  Object.entries(target).filter(([_, v]) => v !== undefined),
                );
                setDoc(doc(db, "outboundMails", newMail.id), cleanMailDoc).catch((err) =>
                  handleFirestoreError(err, OperationType.WRITE, `outboundMails/${newMail.id}`),
                );
        ...
        .catch((err) => console.warn("Background Gmail API send notice:", err));
    }

    return newMail;
  };

  const deleteOutboundMail = (id: string) => {
    setOutboundMails((prev) => {
      const updated = prev.filter((m) => m.id !== id);
      saveAll({ outboundMails: updated });
      deleteDoc(doc(db, "outboundMails", id)).catch((err) =>
        handleFirestoreError(err, OperationType.DELETE, `outboundMails/${id}`),
      );
      return updated;
    });
  };

  const clearOutboundMails = () => {
    outboundMails.forEach((m) => {
      deleteDoc(doc(db, "outboundMails", m.id)).catch(() => {});
    });
    setOutboundMails([]);
    saveAll({ outboundMails: [] });
  };
